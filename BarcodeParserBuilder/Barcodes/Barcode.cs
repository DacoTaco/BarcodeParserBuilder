...
public class ReadOnlyFieldCollection(KeyedCollection<string, IBarcodeField> collection) : ReadOnlyCollection<IBarcodeField>(collection)
{
    private readonly KeyedCollection<string, IBarcodeField> _innerCollection = collection ?? throw new ArgumentException($"{nameof(collection)} should not be null");
